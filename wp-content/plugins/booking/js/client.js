        ...
        if ( 
            ...
            if ( jQuery( element ).is( ':visible' ) ) {                                             //FixIn: 7.2.1.12.2
                ...
                if ( element.className.indexOf('wpdev-validates-as-required') !== -1 ){
                    // [m3956: email invalidation patch]
                    if(element.className.indexOf('wpdev-validates-as-email') !== -1 && element.className.indexOf('same_as_') === -1 && jQuery.trim(inp_value) === ''){
                        var m3956emlsuffix = '@horse-club.wdm'; // dummy address here, change it if you need reciepts on it
                        inp_value = (new Date()).getTime()+m3956emlsuffix; // TODO: load phone number here (need some more advanced actions to be performed)
                    }
                    // [/m3956: email invalidation patch]
